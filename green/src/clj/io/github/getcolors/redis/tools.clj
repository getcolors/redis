    ...
    (let [delete? (= :delete (:green/event opts))
      ...
      (case (:status result)
        "present" (let [node (first (get-in result [:cluster :nodes]))
                    ...
                    (if (and (= :rehearse (:green/event opts)) (storage/managed? opts))
                      (storage/read-credentials! ready)
                      ready))
        "destroyed" (if delete? (assoc opts :redis/already-destroyed true :green/exit 0)
                        (assoc opts :green/exit 1 :green/err "compute deployment is destroyed"))
        "absent" (if (and delete? (or (storage/managed? opts) (managed-backend? opts)))
                   (assoc opts :redis/already-destroyed true :green/exit 0)
        ...
        (if (and delete? (managed-backend? opts))
          (assoc opts :redis/finalize-only true :green/exit 0)
          (assoc opts :green/exit 1 :green/err "compute inspection refused; existing owned state is required"))))
...
(defn ansible-local-data
  ...
  Standard §6)."
  [opts]
  (assoc (dissoc opts storage/credentials-key)
...
(defn ansible-data
  ...
  needs it: not in `.colors/`, not in a golden, not in this map. The managed
  storage output is dropped for the same reason."
  [opts]
  (assoc (dissoc opts storage/credentials-key)
...
(defn play-env
  ...
  [opts credentials?]
  (merge {"ANSIBLE_HOST_KEY_CHECKING" "False"}
         (when (and credentials? (storage/managed? opts)) (storage/credential-env opts))))
