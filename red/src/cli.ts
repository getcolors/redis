...
import { redisWorkflow } from "./workflow.ts";
...
export const usage =
  ...
  "  create    provision the machine, converge Redis, run the gates\n" +
...
export async function run(...args: string[]): Promise<Opts> {
  ...
  if (lifecycleCommands.includes(command)) {
    return runCli(redisWorkflow, withFile);
  }
...
export async function exec(args: string[] = Bun.argv.slice(2)): Promise<never> {
  if (lifecycleCommands.includes(args[0] ?? "")) {
    return execCli(redisWorkflow, defaultArgs(args));
  }
