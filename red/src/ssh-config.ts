...
// one place the address is known and because the play already handles the
...
//
// Unlike the keypair, this play is the package's own copy rather than the
// compute library's (standard section 7). The file is shared with every other
// host the operator reaches, so an unrelated change upstream must not be able
// to rewrite it at pin-bump time.
...
// The profile, unchanged. Standard section 2: the profile already keys remote
// state, which is what makes it unique enough to name a host by.
export function hostAlias(opts: Opts): string {
  return String(opts.profile || "redis");
}
...
// $HOME first, the way the local play's `~` resolves it, so the preflight
// reads the file Ansible will edit.
...
export function hostPatterns(line: string): string[] | undefined {
  ...
  return match[1]!.split(/\s+/).filter((pattern) => pattern.trim().length > 0);
}
...
// block is written at the top of the file, so it would land above that option
...
// The standard's never-adopt rule (section 5). A hand-written `Host <profile>`
...
// The standard's placement rule (section 5), in the one shape that cannot be
...
// `~/.ssh/config` at all (section 6). The checks are injectable so tests can
// cover the refusal without a doctored home directory.
