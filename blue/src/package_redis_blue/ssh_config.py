...
compute library's (standard §7). The file is shared with every other host the
operator reaches, so an unrelated change upstream must not be able to rewrite
it at pin-bump time.
"""
...
def host_alias(opts: dict) -> str:
    ...
    return opts.get("profile") or "redis"
...
def host_patterns(line: str) -> list[str] | None:
    ...
    match = re.fullmatch(r"\s*Host\s+(.*?)\s*", str(line), re.IGNORECASE)
    if not match:
        return None
...
def leading_option_line(lines: list) -> int | None:
    ...
    for n, line in enumerate(lines, start=1):
        ...
        if not trimmed or trimmed.startswith("#"):
            continue
        if re.fullmatch(r"\s*(Host|Match)\s+.*", str(line), re.IGNORECASE):
            return None
        return n
    return None
