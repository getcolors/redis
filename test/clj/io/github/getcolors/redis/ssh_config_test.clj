(ns io.github.getcolors.redis.ssh-config-test
...
(deftest alias-is-the-profile
  (is (= "redis-fixture" (ssh-config/host-alias (fixture)))))
...
(deftest identity-file-keeps-the-tilde
  ...
  (is (= "~/.ssh/redis-fixture" (ssh-config/identity-file (fixture))))
...
(deftest the-marker-is-the-alias-alone
  ...
  ;; too would repeat it: "# BEGIN redis redis-vultr".
  (is (= "# BEGIN redis-vultr ANSIBLE MANAGED BLOCK"
         (ssh-config/begin-marker "redis-vultr")))
  (is (= "# END redis-vultr ANSIBLE MANAGED BLOCK"
         (ssh-config/end-marker "redis-vultr"))))

;; §5 never adopt

(deftest a-foreign-stanza-is-found
  (let [lines ["Host other" "    HostName 192.0.2.1" "" "Host redis-fixture"]]
    (is (= 4 (ssh-config/foreign-stanza-line lines "redis-fixture")))))

(deftest our-own-block-is-not-foreign
  (let [alias "redis-fixture"
...
(deftest a-block-under-a-retired-marker-is-foreign
  ;; The superseded `# BEGIN redis <alias>` marker is gone, so a block
  ...
  (let [alias "redis-vultr"
        lines [(str "# BEGIN redis " alias " ANSIBLE MANAGED BLOCK")
               (str "Host " alias)
               (str "# END redis " alias " ANSIBLE MANAGED BLOCK")]]
    (is (= 2 (ssh-config/foreign-stanza-line lines alias)))))

(deftest a-multi-pattern-host-line-counts
  (is (= 1 (ssh-config/foreign-stanza-line ["Host web redis-fixture db"]
                                           "redis-fixture"))))

(deftest an-unrelated-file-is-left-alone
  (is (nil? (ssh-config/foreign-stanza-line ["Host build" "Host redis-other"]
                                            "redis-fixture"))))
...
(deftest the-local-play-renders-no-address
  ...
  (let [data (tools/ansible-local-data (assoc (fixture) :ip "203.0.113.7"))]
    ...
    (is (= "~/.ssh/redis-fixture" (:ssh-config-identity-file data)))))
...
(deftest the-local-stage-renders-three-files
  (let [targets (map #(str (:target %)) (tools/ansible-local-specs (fixture)))]
    ...
    (is (every? #(str/includes? % "redis-ansible-local") targets))))
...
(deftest create-writes-the-block-after-compute-and-before-convergence
  (is (= [:redis/ssh-config]
         (vec (rest (workflow/wire-fn :redis/infrastructure {:green/event :create})))))
  (is (= [:redis/ansible]
         (vec (rest (workflow/wire-fn :redis/ssh-config {:green/event :create}))))))
...
(deftest delete-removes-the-block-before-the-destroy
  ...
  (is (= [:redis/ssh-config]
         (vec (rest (workflow/wire-fn :redis/ansible {:green/event :delete})))))
  (is (= [:redis/infrastructure]
         (vec (rest (workflow/wire-fn :redis/ssh-config {:green/event :delete})))))
  (is (= [:redis/ssh-cleanup]
         (vec (rest (workflow/wire-fn :redis/infrastructure {:green/event :delete}))))))
